...
-- Creating a Common Table Expression (CTE) to calculate the average salary and job satisfaction for each row
WITH CorrelationData AS (
    SELECT
        CAST(e.Salary AS DECIMAL(18, 2)) AS Salary,  -- Cast Salary to DECIMAL to avoid overflow
        CAST(p.JobSatisfaction AS DECIMAL(18, 2)) AS JobSatisfaction,  -- Cast JobSatisfaction to DECIMAL to avoid overflow
        AVG(CAST(e.Salary AS DECIMAL(18, 2))) OVER() AS AvgSalary,  -- Calculate the average Salary using DECIMAL
        AVG(CAST(p.JobSatisfaction AS DECIMAL(18, 2))) OVER() AS AvgSatisfaction  -- Calculate the average JobSatisfaction using DECIMAL
    FROM Employee e  -- From the Employee table
    JOIN PerformanceRating p ON e.EmployeeID = p.EmployeeID  -- Joining the Employee table with PerformanceRating table using EmployeeID
)

-- Now we calculate the correlation using the formula for Pearson correlation coefficient
SELECT
    -- Calculating the Pearson correlation between Salary and JobSatisfaction
    SUM((Salary - AvgSalary) * (JobSatisfaction - AvgSatisfaction)) /  -- Covariance part: Sum of the products of the deviations from the mean
    (
        -- Dividing by the product of the standard deviations of Salary and JobSatisfaction
        SQRT(SUM(POWER(Salary - AvgSalary, 2)) *  -- Standard deviation of Salary
        SUM(POWER(JobSatisfaction - AvgSatisfaction, 2)))  -- Standard deviation of JobSatisfaction
    ) AS Correlation_Salary_Satisfaction  -- Final Pearson correlation result between Salary and Job Satisfaction
FROM CorrelationData;  -- Use the CTE (CorrelationData) for the calculation
